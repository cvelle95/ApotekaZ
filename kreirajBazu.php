...
<h3 style="align:center;">Na Vasem serveru ne postoji baza podataka,kliknite na generisi za pravljenje baze</h3>
<form action="kreirajBazu.php" method="post">
    <input type="submit" name="generisi" value="GENERISI">
</form>
...
<?php
    ...
    $dataBase = new mysqli($host,$user,$pass) or die("Neuspesna konekcija na bazu");
    if ($dataBase->connect_errno) {
       printf("Connect failed: %s\n", $dataBase->connect_error);
       exit();
   }
   if(isset($_POST['generisi'])){
    $querry = "CREATE DATABASE apotekaz";

    if ($dataBase->query($querry) === TRUE) {
          $sqlCreate = "SET NAMES utf8mb4;
          ...
          $dbs = new mysqli($host,$user,$pass,$db) or die("Neuspesna konekcija na bazu");
          $dbs->multi_query("$sqlCreate");
          ...
          echo "Error creating database: " . $dbs->error;
        } 
   }
